...
class Learner:
    ...
    def _build_model(self):
        ...
        regressor.add(
            Dense(self.firstHidden, input_dim=self.state_size, activation='relu'))
        regressor.add(Dense(self.secondHidden, activation='relu'))
        regressor.add(Dense(self.action_size, activation='linear'))
    ...
    def replay(self):
        ...
        for state, action, reward, next_state in minibatch:
            ...
            target = reward + self.gamma * \
                np.max(self.regressor.predict(next_state)[0])
